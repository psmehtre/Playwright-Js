...
test.describe.serial('Products API Automation', () => {
  ...
  test('Should retrieve the created product', async ({ request }) => {
    expect(productId).toBeDefined();  // Ensure productId is not undefined

    const response = await request.get(`${API_BASE_URL}/${ACCOUNT_ID}/products/${productId}`, { headers });
  ...
  test('Should list all products', async ({ request }) => {
    const response = await request.get(`${API_BASE_URL}/${ACCOUNT_ID}/products`, { headers });
    expect(response.status()).toBe(200);
  ...
  test('Should delete the product', async ({ request }) => {
    ...
    const response = await request.delete(`${API_BASE_URL}/${ACCOUNT_ID}/products/${productId}`, { headers });
    expect(response.status()).toBe(204);
  });

  // ❌ Negative Test Case: Retrieve a deleted product
  test('Should return 404 for a deleted product', async ({ request }) => {
    expect(productId).toBeDefined();

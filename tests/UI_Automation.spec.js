...
test.describe.serial("Keygen UI Automation", () => {
    ...
    test.beforeAll(async ({
        ...
        await page
            ...
            .fill("priyadarshan.m@uniacco.com", {
                delay: 100,
            });
    ...
    test("Dashboard Interactions", async () => {
        // await page.screenshot({ path: 'dashboard_after_login.png' })
        await expect(page.locator("//h1[@class='header']")).toContainText(
            "Dashboard"
        );
        await expect(
            page.locator("//h5[normalize-space()='Dashboard']")
        ).toBeVisible();
    ...
    test("Create Product", async () => {
        ...
        await page
            .locator("//select[@id='distributionStrategy']")
            .selectOption("OPEN");
        ...
        await expect(page).toHaveURL(
            /https:\/\/app\.keygen\.sh\/products\/[a-zA-Z0-9]+/
        );
    ...
    test("Delete Product", async () => {
        ...
        const extractedText = fullText
            .trim()
            .replace(/^Please type\s+(.+?)\s+to confirm\.$/s, "$1");

        // Ensure extractedText is a string before filling the input
        if (extractedText) {
            await page
                .locator(
                    "//p[contains(text(),'Please type')]/following-sibling::input[@type='text']"
                )
                .fill(extractedText);
            await page
                .getByRole("button", {
    ...
    test("Create User", async () => {
        ...
        await page
            .locator("form div")
            .filter({
                hasText: "Password help_center View",
            })
            .getByRole("link")
            .press("Tab");
        ...
        await expect(page).toHaveURL(
            /https:\/\/app\.keygen\.sh\/users\/[a-zA-Z0-9]+/
        );
